...
% crescita dal 2005 al 2008 (vedi idrogramma...) e successiva erosione massiccia
% trend spaziali: alcuni tratti sono più soggetti ad un certo cambiamento?
% zone con alta erosione
% qualche grafico, magari normalizzato per le isole inizialmente presenti in un tratto (puoi fare operazioni con le colonne direttamente con pgfplot)

% preludio: forse è la vegetazione più giovane quella che viene asportata? Qual è la composizione delle isole in termini di età?
...
% le piante crescono diversamente in base alle condizioni ambientali (cita articolo Gurnell?) → definire un'età in base al tempo di osservazione è un approccio limitato (qualche altro lavoro simile)
% la vegetazione può essere distinta da satellite solamente quando occupa una certa area. Si è ritenuto che al momento di una prima osservazione la vegetazione abbia \SI{3}{\anni}
...
\medskip
Per quanto precedentemente esposto, l'approccio di definire un'età in base al periodo di osservazione è limitato, ma è sufficiente per gli scopi della ricerca.
